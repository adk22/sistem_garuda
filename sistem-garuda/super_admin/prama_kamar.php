...
if (isset($_GET['edit'])) {
    $edit_mode = true;
    $id = $_GET['edit'];
    $result = mysqli_query($conn, "SELECT * FROM prama_kamar WHERE id=$id");
    $edit_data = mysqli_fetch_assoc($result);
}

// Handle simpan data (edit atau tambah)
if (isset($_POST['simpan'])) {
    ...
    if (isset($_POST['id'])) {
        $id = $_POST['id'];
        if ($gambar != '') {
            $query = "UPDATE prama_kamar SET tipe='$tipe', nama_kamar='$nama_kamar', deskripsi='$deskripsi',
        ...
        $query = "INSERT INTO prama_kamar (tipe, nama_kamar, deskripsi, fasilitas, jumlah_tamu,
    ...
    header("Location: prama_kamar.php");
    exit();
}

if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
    $data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT gambar FROM prama_kamar WHERE id=$id"));
    if ($data && file_exists("gambar_prama/" . $data['gambar'])) {
        unlink("gambar_prama/" . $data['gambar']);
    }
    mysqli_query($conn, "DELETE FROM prama_kamar WHERE id=$id");
    header("Location: prama_kamar.php");
    exit();
}

$result = mysqli_query($conn, "SELECT * FROM prama_kamar ORDER BY tipe ASC, id DESC");
?>
...
<head>
  <meta charset="UTF-8">
  <title>Kelola Kamar Prama Hotel</title>
...
<body class="bg-gray-50 p-6">
  ...
  <h1 class="text-3xl font-bold text-center mb-6">Kelola Kamar - Prama Hotel</h1>
        ...
        <tbody>
          <?php while ($row = mysqli_fetch_assoc($result)) : ?>
            <tr class="border-b">
              ...
              <td class="p-2">
                <?php if ($row['gambar']) : ?>
                  <img src="gambar_prama/<?= $row['gambar'] ?>" class="h-12 w-20 object-cover rounded">
                <?php else : ?>
